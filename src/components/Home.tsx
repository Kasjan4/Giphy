...
const Home = () => {
      ...
      axios.get(`https://api.giphy.com/v1/gifs/${mode}?api_key=6vSx4w8I68CeIv4A0q1QycYQOEZkZdZN&q=${searchTerm}&limit=21&offset=${page}`)
        ...
        .then((resp) => {
          if (page === 0) {
            ...
            let updatedGifs: any = { ...gifs };
            let newGifs: [] = resp.data.data;
            newGifs.forEach((gif: object) => updatedGifs.data.push(gif));
            setGifs(updatedGifs);
            setLoaded(true);
  ...
  function loadImage(index: number): void {
    let updatedIndex: boolean[] = [...imageLoading];
    updatedIndex[index] = true;
    setImageLoading(updatedIndex);
  }

  function smoothScroll(target: any, duration: number) {
    const element: any = document.querySelector(target);
    const targetPosition: number = Math.round(element.getBoundingClientRect().top - 80);
    const startPosition: number = Math.round(window.pageYOffset);
  ...
  return (
    <main>
      ...
      <div className="promo-container">
        <img id="promo" src={`${imagePath}promo.gif`} alt="promo" />
      </div>
